          ...
          <section className='border-b h-8'>
            <Container className='h-full flex items-center justify-between'>
              <div className='flex gap-4'>
                ...
                {LEFT_NAV_ITEMS.map((item) =>
                  item.to === '#' ? (
                    <button
                      key={item.name}
                      onClick={() => toast.info(`${item.name} feature coming soon!`)}
              ...
              <div className='flex gap-4 items-center'>
                ...
                {payload?.role === 'admin' && <AdminMenu />}
                <button onClick={() => toast.info('Notifications coming soon!')} className='hover:opacity-70'>
                  <Bell className='size-4' />
                </button>
                <button onClick={() => toast.info('Shopping cart coming soon!')} className='hover:opacity-70'>
...
function MyEbayMenu() {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className='font-medium text-sm outline-none'>
        My eBay
      </DropdownMenuTrigger>
      <DropdownMenuContent className='w-48 h-fit' align='end'>
        {MY_EBAY_ITEMS.map((item) => (
          <DropdownMenuItem key={item.label} asChild className='cursor-pointer'>
            <Link to={item.to.startsWith('/') ? item.to : `/my-ebay/${item.to}`}>
              {item.label}
            </Link>
          </DropdownMenuItem>
        ))}
