...
const TRIGGERS = [
    { value: 'first_message', label: 'Buyer nhắn lần đầu', description: 'Gửi khi buyer gửi tin nhắn đầu tiên' },
    { value: 'keyword_shipping', label: 'Hỏi về shipping', description: 'Khi buyer hỏi về vận chuyển' },
    { value: 'keyword_size', label: 'Hỏi về size', description: 'Khi buyer hỏi về kích thước' },
    { value: 'keyword_condition', label: 'Hỏi về condition', description: 'Khi buyer hỏi về tình trạng sản phẩm' },
    { value: 'after_hours', label: 'Ngoài giờ làm việc', description: 'Gửi khi ngoài giờ làm việc' },
];
...
export default function AutoReplySettings() {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            toast.success('Template updated! Requires admin re-approval.');
            setDialogOpen(false);
            resetForm();
            fetchTemplates();
        } catch (err: any) {
        ...
        <div className="p-6 max-w-4xl mx-auto space-y-6">
            <div>
                ...
                <p className="text-muted-foreground mt-2">
                    Tạo tin nhắn tự động để phản hồi nhanh cho buyers
                </p>
            </div>
            ...
            <Alert>
                ...
                <AlertDescription>
                    <strong>Lưu ý:</strong> Tất cả auto-reply templates phải được admin duyệt trước khi sử dụng.
                    Tin nhắn vi phạm chính sách sẽ bị từ chối.
                </AlertDescription>
            </Alert>
                ...
                <CardHeader>
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <CardDescription>Quản lý các template phản hồi tự động</CardDescription>
                        </div>
                ...
                <CardContent>
                    {templates.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                            <p>Chưa có template nào. Tạo template đầu tiên!</p>
                        </div>
                    ) : (
                                            ...
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                {template.reviewedByAdmin ? (
                                                    ...
                                                    <Badge variant="destructive">
                                                        ⚠ Flagged: {template.flagReason}
                                                    </Badge>
                                                ) : (
                                                    <Badge variant="secondary">Pending Review</Badge>
                                                )}
                                                {template.delaySeconds > 0 && (
                                                    <Badge variant="outline">
                                                        Delay: {template.delaySeconds}s
                                                    </Badge>
                                                )}
                                            </div>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>
                            {editingTemplate ? 'Edit Template' : 'Create Auto-Reply Template'}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            />
                            <p className="text-xs text-muted-foreground mt-1">
                                Delay before sending (0-300 seconds)
                            </p>
                        </div>
                        ...
                        <Alert>
                            ...
                            <AlertDescription className="text-xs">
                                Template sẽ được gửi đến admin để duyệt. Không được chứa số điện thoại, email,
                                hoặc link ngoài platform.
                            </AlertDescription>
                        </Alert>
                    </div>
