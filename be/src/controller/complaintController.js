...
const createComplaint = async (req, res) => {
  try {
    ...
    // Check missing fields globally before sending to logic
...
const getComplaintDetail = async (req, res) => {
  try {
    ...
    const me = req.user;
    const isBuyer = r.buyer && (r.buyer._id ? r.buyer._id.toString() === me._id.toString() : r.buyer.toString() === me._id.toString());
    const isSeller = r.seller && (r.seller._id ? r.seller._id.toString() === me._id.toString() : r.seller.toString() === me._id.toString());
...
const handleComplaintBySeller = async (req, res) => {
  try {
    ...
    const { note } = req.body;
    const complaint = await complaintService.sellerReplyComplaint(req.user._id, id, note);
...
const sendToAdmin = async (req, res) => {
  try {
    ...
    const { note } = req.body;
    const complaint = await complaintService.escalateToAdmin(req.user._id, id, note);
...
const adminHandle = async (req, res) => {
    return res.status(400).json({ message: "Deprecated. Use /resolve instead." });
};
...
const adminResolveComplaint = async (req, res) => {
  try {
    ...
    const complaint = await complaintService.adminResolveComplaint(req.user._id, id, resolution, note);
